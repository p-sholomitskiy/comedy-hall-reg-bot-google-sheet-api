...
import { MyContext, SheetOption } from './bot.types.js';
import { Telegraf, Context, Markup, session } from 'telegraf';
import {
  bookingDeletedResponses,
  bookingsFoundResponses,
  bookingSuccessMessages,
  bookingUpdatedResponses,
  finalSuccessMessage,
  incorrectNameResponses,
  incorrectPhoneResponses,
  incorrectSeatsResponses,
  nameRequestMessages,
  noAvailableSeatsMessages,
  noBookingsResponses,
  noEventSelectedMessages,
  phoneRequestMessages,
...
bot.on('text', async (ctx) => {
  ...
  switch (state.step) {
    case 'input_phone':
      ...
      state.allTablesData = await getAllSheetsData(
        SPREADSHEET_ID,
        sheetProperties,
      );
      state.allTablesData = filterByXMarkInPartyName(state.allTablesData);

      const userBookings = state.allTablesData.filter((sheet) =>
        sheet.bookings.some((record) => record.phone === state.phone),
      );
      ...
      userBookings.forEach((sheet) => {
        const totalSeats = sheet.bookings.reduce(
          (sum: number, record: IBookingRow) =>
            record.phone === state.phone ? sum + Number(record.seats) : sum,
          0,
        );
      ...
      const selectableSheets = state.allTablesData.filter(
        (sheet: ISheetData) =>
          !userBookings.some((record) => record.sheetName === sheet.sheetName),
      );
      ...
      if (state.action === 'delete' || state.action === 'edit') {
        ...
        const freeSheets = selectableSheets.filter((sheet: ISheetData) => {
          return !userBookings.some(
            (record) => record.sheetName === sheet.sheetName,
          );
        });
        ...
        state.optionsForSelect = new Map();
        freeSheets.forEach((sheet: ISheetData) => {
          state.optionsForSelect?.set(sheet.sheetId, {
            index: sheet.sheetId,
...
bot.action(/hookah_(yes|no)/, async (ctx) => {
  ...
  try {
    if (state.action === 'edit') {
      ...
      ctx.reply(
        `${getRandomMessage(bookingSuccessMessages)}\n\n${finalSuccessMessage}`,
        mainMenu,
      );
      ctx.replyWithDocument({
        source: 'assets/rules.pdf',
...
bot.action(/^select_(\d+)$/, async (ctx) => {
  ...
  if (!state.optionsForSelect) return;
