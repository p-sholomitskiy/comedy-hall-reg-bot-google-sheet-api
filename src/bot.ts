...
interface GoogleSheetApiCustomPostSuccess {
  ok: true;
  sheetId: number | string;
}
...
interface GoogleSheetApiSheetRow {
  date: string;
  name: string;
  phone: string;
  seats: number;
...
interface GoogleSheetApiSheet {
  sheetName: string;
  sheetId: number;
  party: string;
  available: number;
  data: GoogleSheetApiSheetRow[];
}

type GoogleSheetApiGetResponse = GoogleSheetApiSheet[];
...
async function getAllSheets() {
  ...
  const json: GoogleSheetApiGetResponse = await resp.json();
