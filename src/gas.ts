  ...
  sheets.forEach((sheet) => {
    ...
    if (sheetName.includes(':')) {
      ...
      try {
        ...
        Logger.log(
          `Не удалось переименовать лист "${sheetName}": ${e.message}`
        );
      }
    }
  });
...
function doGet(e: any) {
  ...
  return ContentService.createTextOutput(newCache || '[]').setMimeType(
    ContentService.MimeType.JSON
  );
}
...
function doPost(e: any) {
  ...
  try {
    ...
    bookings.forEach((record: any) => {
      try {
        Logger.log(record);
        const sheet = ss.getSheetById(record.sheetId);
        if (!sheet) throw { ...ERRORS.NO_SHEET_ID };

        const values = sheet.getDataRange().getValues();
        let available = Number(sheet.getRange('L1').getValue());

        if (record.action === 'delete') {
          if (!record.phone) throw { ...ERRORS.NO_PHONE };
          let deletedCount = 0;
          ...
          if (!record.phone) throw { ...ERRORS.NO_PHONE };
          if (!record.seats) throw { ...ERRORS.NO_SEATS };
          let updatedCount = 0;
          ...
          if (exist) throw { ...ERRORS.DUPLICATE };
          if (record.seats > available)
            throw { ...ERRORS.SEATS_CHANGED, available };
          const newRow = [
            record.date || '',
 ...
 */
function formatCell(value: any, ss: any) {
