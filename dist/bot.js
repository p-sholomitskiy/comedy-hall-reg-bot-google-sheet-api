...
bot.on('text', async (ctx) => {
    ...
    switch (state.step) {
        ...
        case 'input_phone':
            ...
            //   : await getAllSheets();
            const availableSheets = allSheets.filter((sheet) => !userBookings.some((b) => b.sheetName === sheet.sheetName) &&
                Number(sheet.available) > 0);
            // проверяем, доступны ли кальяны на всех выбранных листах
            const allHookahAvailable = availableSheets.every((sheet) => Number(sheet.hookah) > 0);
