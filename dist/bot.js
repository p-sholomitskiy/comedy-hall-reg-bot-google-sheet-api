...
bot.on('text', async (ctx) => {
  ...
  switch (state.step) {
    ...
    case 'input_phone':
      ...
      const userBookings = allSheets.filter((sheet) =>
        sheet.data.some((row) => row.phone === state.phone),
      );
      ...
      const selectableSheets = allSheets.filter(
        (sheet) => !userBookings.some((b) => b.sheetName === sheet.sheetName),
      );
      ...
      if (state.action === 'delete' || state.action === 'edit') {
        ...
        const freeSheets = selectableSheets.filter((sheet) => {
          return !userBookings.some((b) => b.sheetName === sheet.sheetName);
        });
...
async function sendBooking(ctx) {
  const state = ctx.session;
